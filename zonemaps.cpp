...
std::vector<T> zonemap<T>::query(T low, T high){
    ...
    for(const auto&z : zones){
        // If range does not fit the requirements of the currentzone then we will skip it.
        if(z.max < low || z.min > high){
            continue;
        }
        // Binary search for the first element greater than or equal to the low parameter
        auto start_it = std::lower_bound(z.elements.begin(), z.elements.end(), low);
        
        // Binary search for the last element less than or equal to the high parameter
        auto end_it = std::upper_bound(z.elements.begin(), z.elements.end(), high);

        // If it fits within the range we will add it to our result vector
